...
if (isset($_GET['pr'])) {
    ...
    $stmt = $connection->prepare("SELECT kategoria FROM JS WHERE nazov = ?");
    ...
    if (!empty($category)) {
        $stmt = $connection->prepare("SELECT COUNT(*) FROM JS WHERE kategoria = ?");
        ...
        if ($count > 1) {
            $stmt = $connection->prepare("SELECT nazov FROM JS WHERE kategoria = ? ORDER BY nazov");
            $stmt->bind_param("s", $category);
            $stmt->execute();
