...
if ($isTeacher) {
    ...
    echo '<button type="button" class="btn btn-primary float-end" data-bs-toggle="modal" data-bs-target="#addHtmlModal"><i class="fa-solid fa-plus"></i> Pridať príklad</button>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
}
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<div class="col-md-12"><div class="card-hover card-text mb-3 text-center" onclick="event.preventDefault(); window.location.href = \'html_page_priklad.php?pr=' . $nazov . '\';"><div class="card-body"><p class="card-title">' . $nazov . '</p></div></div></div>';
    }
    echo '</div></div></div>';
} else {
