<div class="sidebar" id="mySidebar">
    <h2><strong>JavaScript</strong></h2>
    <div class="sidebar-container">
        <ul class="nav flex-column side-menu">
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-api1">API</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-ajax1">AJAX</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-grafy1">Grafy</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-menu1">Navigačné menu</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-prepinanie-css1">Prepínanie CSS štýlu</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-vyskakovacie-okna1">Vyskakovacie okná</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-kalkulacka1">Kalkulačka</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-hodiny1">Hodiny</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-drag-and-drop1">Drag and Drop</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-galeria1">Galéria</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-tabulky1">Tabuľky</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?pr=js-vyhladavanie1">Vyhľadávanie</a>
            </li>
        </ul>
    </div>

    <button id="sidebarToggle" class="btn btn-dark">
        <i class="fas fa-arrow-right"></i>
    </button>
</div>